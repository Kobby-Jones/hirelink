...
// ── Types ──────────────────────────────────────────────────────────────────
type NodeType = "entry" | "page" | "step" | "logic" | "error" | "action" | "store" | "storage" | "admin" | "stage" | "infra";
...
const TYPE_STYLES: Record<NodeType, NodeStyle> = {
...
function EdgeLine({ from, to, label, dashed, highlight }: EdgeLineProps) {
...
function NodeBox({ node, highlight, onClick }: NodeBoxProps) {
...
export default function HireLinkFlowchart() {
  ...
  return (
    <div style={{
      ...
      {/* Legend */}
      <div style={{ maxWidth: 1060, margin: "0 auto 20px", display: "flex", flexWrap: "wrap", gap: 8 }}>
        {LEGEND.map(l => {
      ...
      {active && (() => {
        ...
        return (
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, color: s.dot, fontFamily: "JetBrains Mono", marginBottom: 4 }}>
                {n.type.toUpperCase()}
              </div>
